...
def load_course_data():
    ...
    course_file = os.path.join(
        BASE_DIR, "EXAMS_INTERNAL", "ND", "ND-COURSES", "course-code-creditUnit.xlsx"
    )
    ...
    try:
        ...
        for sheet in xl.sheet_names:
            ...
            if not all(col in df.columns for col in expected):
                print(
                    f"⚠️ Sheet '{sheet}' missing expected columns {expected} — skipped"
                )
            ...
            for code in codes:
                ...
                normalized_codes_variants.extend(
...
def find_course_title(course_code, course_titles_dict, course_code_to_title):
    ...
    for strategy in matching_strategies:
        ...
        if strategy in course_titles_dict:
            print(
                f"✅ Found course title for '{original_code}' -> '{strategy}': {course_titles_dict[strategy]}"
            )
        ...
        if strategy in course_code_to_title:
            print(
                f"✅ Found course title for '{original_code}' -> '{strategy}': {course_code_to_title[strategy]}"
            )
...
def load_previous_gpas(mastersheet_path, current_semester_key):
    ...
    all_student_data = {}
    current_year, current_semester_num, _, _, _, current_semester_name = (
        get_semester_display_info(current_semester_key)
    )
                ...
                for idx, row in df.iterrows():
                    ...
                    if (
                        ...
                        try:
                            ...
                            print(
                                f"📊 Loaded GPA for {exam_no}: {gpa} with {credits} credits from {semester}"
                            )
...
def process_carryover_results(
    ...
    # Load course data
    semester_course_titles, semester_credit_units, course_code_to_title = (
        load_course_data()
    )
    ...
    try:
        ...
        mastersheet_df = pd.read_excel(
            temp_mastersheet_path, sheet_name=semester_key, header=5
        )

        print(
            f"✅ Files loaded - Resit: {len(resit_df)} rows, Mastersheet: {len(mastersheet_df)} students"
        )
        ...
        print(
            f"📝 Exam columns - Resit: '{resit_exam_col}', Mastersheet: '{mastersheet_exam_col}'"
        )
        ...
        for idx, resit_row in resit_df.iterrows():
            ...
            if exam_no in cgpa_data:
                student_record["CURRENT_CGPA"] = calculate_cgpa(
                    cgpa_data[exam_no], student_record["CURRENT_GPA"], current_credits
                )
            else:
            ...
            if exam_no in cgpa_data:
                ...
                for i, prev_semester in enumerate(student_gpa_data["semesters"]):
                    sem_num = get_semester_display_info(prev_semester)[
                        5
                    ]  # Get "Semester X"
                ...
                if col in mastersheet_df.columns:
                    ...
                    if original_score_val < pass_threshold:
                        ...
                        course_title = find_course_title(
                            col,
                            semester_course_titles.get(semester_key, {}),
                            course_code_to_title,
                        )
                        # Get credit unit
                        credit_unit = semester_credit_units.get(semester_key, {}).get(
                            col, 0
                        )
            ...
            if student_record["RESIT_COURSES"]:
                ...
                print(
                    f"✅ {exam_no}: {len(student_record['RESIT_COURSES'])} resit courses, CGPA: {student_record['CURRENT_CGPA']}"
                )
        ...
        if carryover_data:
            ...
            carryover_mastersheet_path = generate_carryover_mastersheet(
                ...
                cgpa_data,
                semester_course_titles.get(semester_key, {}),
                semester_credit_units.get(semester_key, {}),
            )
...
def generate_carryover_mastersheet(
    ...
    for course in sorted(all_courses):
        ...
        title_row.extend(
            [course_title, course_title]
        )  # Use title for both original and resit columns
    ...
    for course in sorted(all_courses):
        ...
        credit_row.extend(
            [f"CU: {credit_unit}", f"CU: {credit_unit}"]
        )  # Credit unit for both original and resit columns
    ...
    for course in sorted(all_courses):
        ...
        for offset in [0, 1]:
            cell = ws.cell(
                row=5, column=start_col + offset
            )  # CHANGED: Row 5 for course titles
    ...
    for row in [5, 6, 7]:  # CHANGED: Rows 5, 6, 7
        ...
        for col in range(
            len(headers) - 2, len(headers) + 1
        ):  # GPA Semester 4, CGPA, REMARKS
            cell = ws.cell(row=row, column=col)
            cell.fill = PatternFill(
                start_color="366092", end_color="366092", fill_type="solid"
            )
    ...
    for col_idx, column in enumerate(ws.columns, 1):
            ...
            try:
                if cell.value is not None:
                    ...
                    if cell.row == 5 and cell.alignment.text_rotation == 90:
                        ...
                        cell_length = max(
                            cell_length, 10
                        )  # Minimum width for rotated text
                    ...
                    if isinstance(cell.value, (int, float)) and not isinstance(
                        ...
                        cell_length = max(
                            cell_length, 8
                        )  # Ensure enough space for numbers
        ...
        if col_idx == 1:  # S/N
        ...
        elif col_idx >= 4 and col_idx <= (
            4 + (3 if semester_key == "ND-SECOND-YEAR-SECOND-SEMESTER" else 0)
        ):
            # GPA columns
            adjusted_width = 15
            ...
            adjusted_width = min(
                max(adjusted_width, 12), 25
            )  # Course columns between 12-25 width
...
def main():
    ...
    clean_dir = os.path.join(
        BASE_DIR, "EXAMS_INTERNAL", "ND", set_name, "CLEAN_RESULTS"
    )
    ...
    if not os.path.exists(clean_dir):
        print(f"❌ Clean directory doesn't exist: {clean_dir}")
        print(
            "💡 Please run the regular result processor first to generate clean results"
        )
        return
    ...
    resit_files = [
        f
        for f in os.listdir(resit_subdir)
    ...
    if not zip_file_path:
        ...
        print(
            "💡 Please run the regular result processor first to generate clean results ZIP"
        )
        return
