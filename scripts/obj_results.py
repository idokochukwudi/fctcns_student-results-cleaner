...
def apply_professional_formatting(excel_file):
    ...
    overall_avg_fill = PatternFill(start_color="FFF2CC", end_color="FFF2CC", fill_type="solid")
...
for set_id, files in grouped_files.items():
    ...
    for file in files:
        ...
        cleaned_df["MAT NO."] = cleaned_df["MAT NO."].astype(str).str.strip().str.upper()
        cleaned_df["FULL NAME"] = cleaned_df["FULL NAME"].astype(str).str.strip().str.title()
        ...
        output_file = os.path.join(output_dir, f"{set_id}_cleaned_{safe_base_name}.xlsx")
