...
def find_latest_mastersheet(clean_dir, set_name):
    ...
    for item in os.listdir(clean_dir):
        if (
            item.startswith(f"{set_name}_RESULT-")
            and not item.startswith("UPDATED_")
...
def load_carryover_results(carryover_dir):
    ...
    for file in os.listdir(carryover_dir):
        if file.endswith(".json") and (
            "co_student" in file.lower() or "carryover" in file.lower()
        ):
...
def create_backup_zip(original_zip_path):
    ...
    try:
        ...
        backup_zip_path = os.path.join(
            os.path.dirname(original_zip_path), backup_zip_name
        )
        ...
        if os.path.exists(backup_zip_path) and os.path.getsize(backup_zip_path) > 0:
            ...
            print(
                f"✅ BACKUP CREATED SUCCESSFULLY: {backup_zip_path} ({backup_size} bytes)"
            )
            return True
        else:
    ...
    try:
        ...
        for col_idx, cell in enumerate(ws[header_row], start=1):
            if cell.value:
                ...
                if re.match(r"^[A-Z]{3}\d{3}$", header_name) or any(
                    course in header_name
                    for course in ["GNS", "NUS", "NUR", "MTH", "CHE", "BIO", "PHY"]
                ):
...
def calculate_student_stats(ws, row_idx, headers, course_columns, header_row):
    ...
    try:
        ...
        gpa = (
            round(total_grade_points / total_credit_units, 2)
    ...
    try:
        ...
        while current_row <= ws.max_row:
            ...
            # Update passed students
                ...
                "PASSED" in cell_str
                and "CARRYOVER" not in cell_str
                and "PROBATION" not in cell_str
            ):
...
def update_cgpa_summary_sheet(wb):
    ...
    try:
        ...
        cgpa_ws.cell(row=1, column=1).value = (
            "UPDATED - Please regenerate CGPA analysis"
        )
    ...
    try:
                ...
                for course_code, new_score in student_updates.items():
                    ...
                    if course_col_idx:
                        ...
                        update_details.append(
                            f"{exam_no} - {course_code}: {old_score}→{new_score}"
                        )
                    else:
        ...
        for row_idx in range(header_row + 1, ws.max_row + 1):
            ...
            stats = calculate_student_stats(
                ws, row_idx, headers, course_columns, header_row
            )
            ...
            for stat_name, col_idx in summary_columns.items():
                if col_idx:
                    ...
                    if (
                        exam_no in updates
                    ):  # Only highlight for students with carryover updates
        ...
        updated_header_row = apply_professional_formatting(
            ws, headers, header_row, "ND-2024"
        )
    ...
    try:
        ...
        if os.path.exists(new_zip_path) and os.path.getsize(new_zip_path) > 0:
            ...
            if os.path.exists(original_zip):
                ...
                print(
                    f"✅ Original ZIP preserved: {original_zip} ({original_size} bytes)"
                )
            else:
...
def main():
    ...
    semester_key = input(
        f"Enter semester key [default: ND-FIRST-YEAR-FIRST-SEMESTER]: "
    ).strip()
    if not semester_key:
        semester_key = "ND-FIRST-YEAR-FIRST-SEMESTER"

    carryover_dir = input(
        f"Enter carryover directory [default: ~/student_result_cleaner/EXAMS_INTERNAL/ND/ND-2024/CLEAN_RESULTS/CARRYOVER_RECORDS]: "
    ).strip()
    if not carryover_dir:
        carryover_dir = os.path.expanduser(
            "~/student_result_cleaner/EXAMS_INTERNAL/ND/ND-2024/CLEAN_RESULTS/CARRYOVER_RECORDS"
        )
    else:
    ...
    # Find mastersheet
    mastersheet_path, temp_dir, original_zip = find_latest_mastersheet(
        clean_dir, set_name
    )
    ...
    if success:
        ...
        if original_zip and temp_dir:
            repack_success = repack_updated_zip(
                clean_dir, temp_dir, original_zip, set_name, semester_key
            )
            if repack_success:
