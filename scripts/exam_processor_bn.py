...
def process_in_non_interactive_mode(params, base_dir_norm):
    ...
    for bn_set in sets_to_process:
        ...
        if not raw_files:
            print(f"⚠️ No raw files in {raw_dir}; skipping {bn_set}")
            continue
        
        print(f"📁 Found {len(raw_files)} raw files in {bn_set}: {raw_files}")
        ...
        for semester_key in semesters_to_process:
            ...
            if semester_files_exist:
                ...
                print(f"⚠️ No files found for {semester_key} in {bn_set}, skipping...")
...
SEMESTER_ORDER = [
    ...
    "BN-FIRST-YEAR-SECOND-SEMESTER",
    "BN-SECOND-YEAR-FIRST-SEMESTER",
    "BN-SECOND-YEAR-SECOND-SEMESTER"
]
...
def get_available_sets(base_dir):
    ...
    if not os.path.exists(bn_dir):
        print(f"❌ BN directory not found: {bn_dir}")
        return []
...
def load_course_data():
    """
    Reads N-course-code-creditUnit.xlsx and returns:
    ...
    for sheet in xl.sheet_names:
        ...
        # Add variations without "BN-" prefix
...
def detect_semester_from_filename(filename):
    ...
    if 'FIRST-YEAR-FIRST-SEMESTER' in filename_upper or 'FIRST_YEAR_FIRST_SEMESTER' in filename_upper or 'FIRST SEMESTER' in filename_upper:
        return "BN-FIRST-YEAR-FIRST-SEMESTER", 1, 1, "YEAR ONE", "FIRST SEMESTER", "BN1"
        ...
        return "BN-FIRST-YEAR-SECOND-SEMESTER", 1, 2, "YEAR ONE", "SECOND SEMESTER", "BN1"
    elif 'SECOND-YEAR-FIRST-SEMESTER' in filename_upper or 'SECOND_YEAR_FIRST_SEMESTER' in filename_upper:
        return "BN-SECOND-YEAR-FIRST-SEMESTER", 2, 1, "YEAR TWO", "FIRST SEMESTER", "BN2"
    elif 'SECOND-YEAR-SECOND-SEMESTER' in filename_upper or 'SECOND_YEAR_SECOND_SEMESTER' in filename_upper:
        return "BN-SECOND-YEAR-SECOND-SEMESTER", 2, 2, "YEAR TWO", "SECOND SEMESTER", "BN2"
        ...
        # Default fallback
        print(
            f"⚠️ Could not detect semester from filename: {filename}, defaulting to BN-FIRST-YEAR-FIRST-SEMESTER")
        return "BN-FIRST-YEAR-FIRST-SEMESTER", 1, 1, "YEAR ONE", "FIRST SEMESTER", "BN1"
...
def load_all_previous_gpas_for_cgpa(
    ...
    if current_semester_num == 1 and current_year == 1:
        ...
        semesters_to_load = ["BN-FIRST-YEAR-FIRST-SEMESTER"]
    elif current_semester_num == 1 and current_year == 2:
...
def process_single_file(
    ...
    output_subdir = os.path.join(output_dir, f"BN_RESULT-{ts}")
    ...
    subtitle_cell = ws["C2"]
    subtitle_cell.value = f"{datetime.now().year}/{datetime.now().year + 1} SESSION  BASIC NURSING {expanded_semester_name} EXAMINATIONS RESULT — {datetime.now().strftime('%B %d, %Y')}"
    ...
    ws.append(
        [f"A total of {passed_all} students passed in all courses registered and are to proceed to Second Semester, BN 1"])
...
def main():
    print("Starting BN Examination Results Processing...")
    ts = datetime.now().strftime(TIMESTAMP_FMT)
    ...
    if should_use_interactive_mode():
        ...
        if not available_sets:
            print(
                f"No SET* directories found in {base_dir_norm}. Nothing to process.")
