...
def create_bn_cgpa_summary_sheet(mastersheet_path, ts, semester_credit_units, set_name="", logo_path=None):
    ...
    try:
        ...
        if not semester_credit_units:
            logger.error("❌ Semester credit units data is empty")
            return None
        ...
        for sheet_name in wb.sheetnames:
            if sheet_name not in BN_SEMESTER_ORDER:
                continue
              
            logger.info(f"📖 Processing sheet: {sheet_name}")
          
            try:
                # FIXED: Dynamic header detection instead of fixed header=5
                ...
                for idx, row in df_raw.iterrows():
                    if idx > 20: # Don't search beyond first 20 rows
                        break
                    row_values = [str(val).strip().upper() for val in row if pd.notna(val)]
                    if any(
                        "EXAMS NUMBER" in val or "EXAM NUMBER" in val or ("EXAM" in val and "NUMBER" in val)
                ...
                if header_row_idx is None:
                    logger.warning(f"⚠️ Could not find header row in {sheet_name}")
                    continue
                ...
                # Validate dataframe
                if df.empty:
                    logger.warning(f"⚠️ No data after reading with header in {sheet_name}")
                    continue
              
                # Clean column names
                df.columns = [
                    str(col).strip() if col is not None else f"Unnamed_{i}"
                ...
                for col in df.columns:
                    ...
                    if "EXAMS NUMBER" in col_str or "EXAM NUMBER" in col_str:
                        exam_col = col
                        break
                ...
                if not exam_col:
                    ...
                    for col in df.columns:
                        col_str = str(col).upper().strip()
                        if "REG" in col_str and "NO" in col_str:
                            exam_col = col
                            break
                ...
                # Validate we still have data after filtering
                if df.empty:
                    logger.warning(f"⚠️ No valid data after filtering in {sheet_name}")
                    continue
                ...
                for col in df.columns:
                    if str(col).strip().upper() == "REMARKS":
                        remarks_col_name = col
                        logger.info(f"✅ Found REMARKS column: '{col}'")
                        break
              
                if not remarks_col_name:
                    ...
                    try:
                        ...
                        if pd.notna(gpa_value):
                            try:
                                ...
                                logger.warning(f"⚠️ Invalid GPA value for {exam_no}: {gpa_value}")
                        ...
                        if sheet_name in semester_credit_units:
                            ...
                            # Default credit value if not found
                            cgpa_data[exam_no]["credits"][sheet_name] = 30
                      
                        # CRITICAL FIX: Capture status from REMARKS column using dynamic lookup
                        if remarks_col_name and remarks_col_name in row.index:
                            ...
                            if pd.notna(remarks_value):
                                ...
                                if students_processed <= 3:
                                    logger.info(f"📝 Status captured: {exam_no} in {sheet_name} -> '{remarks}'")
                            else:
                                ...
                                if pd.notna(cu_failed) and cu_failed == 0:
                                    remarks = "Passed"
                            ...
                            logger.warning(f"⚠️ REMARKS column not accessible for {exam_no} in {sheet_name}")
                  
                    except Exception as row_error:
                        logger.warning(f"⚠️ Error processing row {idx} in {sheet_name}: {row_error}")
                        continue
        ...
        if not cgpa_data:
            logger.warning("⚠️ No CGPA data collected from any semester")
            return None
        ...
        if not summary_data:
            logger.warning("⚠️ No summary data to create dataframe")
            return None
        ...
        # Add S/N column (serial number) as the first column
        summary_df.insert(0, "S/N", range(1, len(summary_df) + 1))
      
        # Convert summary_df back to list of dicts to include S/N
        ...
        # Add document headings - FIXED: Expanded to match analysis sheet exactly
        headers = (
            ["S/N", "EXAM NO", "NAME", "PROB HIST"]  # Moved STATUS to last
        ...
        if logo_path and os.path.exists(logo_path):
            try:
                ...
                logger.warning(f"⚠️ Could not place logo in CGPA_SUMMARY: {e}")
      
        # College name - FIXED: Start from row 1 but merge across columns (expanded to fit)
...
def create_bn_analysis_sheet(mastersheet_path, ts, semester_credit_units, set_name="", logo_path=None):
    ...
    try:
        ...
        for row_idx, row_data in analysis_df.iterrows():
            ws.cell(row=row_idx + start_row + 1, column=1, value=row_data["S/N"])  # Added S/N
            ws.cell(row=row_idx + start_row + 1, column=2, value=row_data["SEMESTER"])
...
def process_bn_single_file(
    ...
    # FIXED: NEW SORTING RULE - Primary: GPA desc, Secondary: AVERAGE desc, Tertiary: TCPE desc
    mastersheet = mastersheet.sort_values(by=["GPA", "AVERAGE", "TCPE"], ascending=[False, False, False]).reset_index(drop=True)
    ...
    ws.merge_cells(f"C3:{last_letter}3")
    set_result_cell = ws["C3"]
    set_result_cell.value = f"{set_name} BASIC NURSING {level_display} {semester_display} RESULT"
    set_result_cell.font = Font(bold=True, size=12, color="000000")
    set_result_cell.alignment = Alignment(horizontal="center", vertical="center")
 
    ws.merge_cells(f"C4:{last_letter}4")
    session_cell = ws["C4"]
    session_cell.value = f"{datetime.now().year}/{datetime.now().year + 1} SESSION — {datetime.now().strftime('%B %d, %Y')}"
    session_cell.font = Font(bold=True, size=12, color="000000")
